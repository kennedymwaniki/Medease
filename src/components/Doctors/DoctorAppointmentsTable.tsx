...
const DoctorAppointmentsTable: React.FC = () => {
  ...
  const pagination = usePagination(filteredData, {
    state: {
      page: 0,
      size: 5, // Show 5 appointments per page
    },
  ...
  const sort = useSort(
    ...
    },
    {
      sortFns: {
        ...
        STATUS: (array) =>
          array.sort((a: any, b: any) => a.status.localeCompare(b.status)),
        PATIENT: (array) =>
          array.sort((a: any, b: any) => {
            const nameA = `${a.patient.user.firstname} ${a.patient.user.lastname}`
            const nameB = `${b.patient.user.firstname} ${b.patient.user.lastname}`
  ...
  const COLUMNS = [
    {
      label: 'Date & Time',
      renderCell: (item: DoctorAppointment) => {
    ...
    },
    {
      ...
      renderCell: (item: DoctorAppointment) => (
        <div className="flex items-center space-x-3">
          <div
            className={`w-8 h-8 rounded-lg flex items-center justify-center ${getTypeIcon(item.title)}`}
          >
    ...
    },
    {
      ...
      sort: { sortKey: 'PATIENT' },
    },
    {
      label: 'Status',
      renderCell: (item: DoctorAppointment) => (
        <div
          className={`inline-flex items-center space-x-2 px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(item.status)}`}
        >
    ...
    <div className="p-6 bg-white">
      ...
      <div className="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div className="text-sm text-gray-600">
          ...
          {doctorData?.appointments.length || 0} appointments
          {search &&
